 ...
 * *exchange* services (swaps, bridges, off-ramps) inside an iOS app. We keep
 * those features on Android and the web build, and hard-disable them on iOS.
 *
 ...
 *  2. Runtime: the Capacitor platform reports `ios` (via `nativePlatform()`).
 *
 * Any new exchange-like feature must be gated here and hidden from iOS
 * before it ships.
 */
...
/** True when swap / bridge / off-ramp features may be shown on this device. */
 ...
 * client render (so iOS never flashes an exchange button), then resolves
 * after mount when the Capacitor platform is known.
 */
