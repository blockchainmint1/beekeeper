...
Branded source art lives in `resources/`:

- `resources/icon.png` — 1024×1024 app icon (Honey hex mark on Hive Navy)
- `resources/splash.png` / `resources/splash-dark.png` — 2732×2732 splash

`bun run cap:assets` (and the Android CI workflow) reads these and generates
every Android density and iOS size. Replace those three files to rebrand.

The `android/` and `ios/` folders are **not** committed — they're regenerated by
`bunx cap add <platform>` in `*:setup` scripts and in CI.
